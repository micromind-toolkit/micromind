...
class Yolov8Neck(nn.Module):
    ...
    def __init__(self, filters=[256, 512, 768], d=1):
        ...
        self.n1 = C2f(
            c1=int(filters[1] + filters[2]), c2=int(filters[1]), n=round(3 * d), shortcut=False
        )
        self.n2 = C2f(c1=int(filters[0] + filters[1]), c2=int(filters[0]), n=round(3 * d), shortcut=False)
        self.n3 = Conv(
            c1=int(filters[0]), c2=int(filters[0]), kernel_size=3, stride=2, padding=1
        )
        self.n4 = C2f(c1=int(filters[0] + filters[1]), c2=int(filters[1]), n=round(3 * d), shortcut=False)
        self.n5 = Conv(
