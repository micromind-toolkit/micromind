...
"""
Export a YOLOv8 PyTorch model to other formats. TensorFlow exports authored by
...
TensorFlow.js:
    $ cd .. && git clone https://github.com/zldrobit/tfjs-yolov5-example.git
...
def try_export(inner_func):
    ...
    def outer_func(*args, **kwargs):
        ...
        try:
            ...
            LOGGER.info(
                f"{prefix} export success ✅ {dt.t:.1f}s,"
...
class Exporter:
    ...
    def __call__(self, model=None):
        ...
        if self.args.half and onnx and self.device.type == "cpu":
            LOGGER.warning(
                "WARNING ⚠️ half=True only compatible with GPU export, use device=0"
            )
        ...
        if edgetpu and not LINUX:
            raise SystemError(
                "Edge TPU export only supported on Linux. See"
        ...
        for k, m in model.named_modules():
            # if isinstance(m, (Detect, Segment)): TODO
            if isinstance(m, (Detect)):
                m.dynamic = self.args.dynamic
                m.export = True
                m.int8 = self.args.int8
                m.format = self.args.format
            elif isinstance(m, C2f) and not any(
                (saved_model, pb, tflite, edgetpu, tfjs)
            ):
                # EdgeTPU does not support FlexSplitV while split provides
                # cleaner ONNX graph
        ...
        LOGGER.info(
            f"\n{colorstr('PyTorch:')} starting from {file} with input "
            f"shape {tuple(im.shape)} BCHW and "
        ...
        if any(f):
            ...
            s = (
                ""
                if square
                else f"WARNING ⚠️ non-PyTorch val requires square images, "
                f"'imgsz={self.imgsz}' will not "
            ...
            LOGGER.info(
                ...
                f"\nPredict:         yolo predict task={model.task} model={f} "
                f"imgsz={imgsz} {data}"
                f"\nValidate:        yolo val task={model.task} model={f} imgsz={imgsz}"
                f" data={self.args.data} {s}"
    ...
    def export_onnx(self, prefix=colorstr("ONNX:")):
        ...
        torch.onnx.export(
            ...
            do_constant_folding=True,  # WARNING: DNN inference with torch>=1.12 may
    ...
    def export_engine(self, prefix=colorstr("TensorRT:")):
        ...
        if self.args.dynamic:
            ...
            if shape[0] <= 1:
                LOGGER.warning(
                    f"{prefix} WARNING ⚠️ --dynamic model requires maximum "
                    f"--batch-size argument"
                )
        ...
        LOGGER.info(
            f"{prefix} building FP"
            f"{16 if builder.platform_has_fast_fp16 and self.args.half else 32}"
            f"engine as {f}"
        )
    ...
    def export_saved_model(self, prefix=colorstr("TensorFlow SavedModel:")):
        ...
        try:
            ...
            tmp = "-macos" if MACOS else "-aarch64" if ARM64 else "" if cuda else "-cpu"
            check_requirements(f"tensorflow{tmp}")
            import tensorflow as tf  # noqa
        check_requirements(
            (
                "onnx",
    ...
    def export_edgetpu(self, tflite_model="", prefix=colorstr("Edge TPU:")):
        ...
        LOGGER.warning(
            f"{prefix} WARNING ⚠️ Edge TPU known bug "
            f"https://github.com/ultralytics/ultralytics/issues/1185"
        )
        ...
        if (
            ...
            LOGGER.info(
                f"\n{prefix} export requires Edge TPU compiler. Attempting "
                f"install from {help_url}"
            )
            sudo = (
    ...
    def _pipeline_coreml(self, model, prefix=colorstr("CoreML Pipeline:")):
        ...
        if MACOS:
            ...
            # img = torch.zeros((*opt.img_size, 3)).numpy()  # img size(320,192,3)
            # iDetection
            out = model.predict({"image": img})
            out0_shape = out[out0.name].shape
            out1_shape = out[out1.name].shape
        else:
            # linux and windows can not run model.predict(), get sizes from pytorch
            # output y
        ...
        # s.append(flexible_shape_utils.NeuralNetworkImageSize(640, 384))
        #   # (height, width)
        # flexible_shape_utils.add_enumerated_image_sizes(spec,
        #   feature_name='image', sizes=s)
        ...
        model.input_description["confidenceThreshold"] = (
            f"(optional) Confidence threshold override (default:"
            f" {nms.confidenceThreshold})"
        )
...
class iOSDetectModel(torch.nn.Module):
    ...
    def forward(self, x):
        """Normalize predictions of object detection model with input size-dependent
        factors."""
