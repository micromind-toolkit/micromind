...
def benchmark(
    ...
    Args:
        model (Union[str, Path], optional): Path to the model file or directory. Default
             is
            Path(SETTINGS['weights_dir']) / 'yolov8n.pt'.
        imgsz (int, optional): Image size for the benchmark. Default is 160.
        half (bool, optional): Use half-precision for the model if True.
            Default is False.
        int8 (bool, optional): Use int8-precision for the model if True.
            Default is False.
        device (str, optional): Device to run the benchmark on, either 'cpu' or 'cuda'.
            Default is 'cpu'.
        hard_fail (Union[bool, float], optional): If True or a float, assert benchmarks
            pass with given metric.
            Default is False.

    Returns:
        df (pandas.DataFrame): A pandas DataFrame with benchmark results for each
    ...
    s = f"\nBenchmarks complete for {name} on {data}"
    s += f"at imgsz={imgsz} ({time.time() - t0:.2f}s)\n{df}\n"
    LOGGER.info(s)
...
class ProfileModels:
    ...
    This class profiles the performance of different models, provided their
    paths. The profiling includes parameters such as
