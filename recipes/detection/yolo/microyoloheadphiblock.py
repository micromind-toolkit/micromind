...
class Microhead(nn.Module):
    def __init__(
            ...
            if number_heads >= 2:
                ...
                if number_heads >= 3:
                    ...
                    self._save.extend(
                        x % layer19.i
                        for x in (
                            [layer19.f] if isinstance(layer19.f, int) else layer19.f
                        )
                        if x != -1
                    )
                    ...
                    self._save.extend(
                        x % layer20.i
                        for x in (
                            [layer20.f] if isinstance(layer20.f, int) else layer20.f
                        )
                        if x != -1
                    )
                    ...
                    self._save.extend(
                        x % layer21.i
                        for x in (
                            [layer21.f] if isinstance(layer21.f, int) else layer21.f
                        )
                        if x != -1
                    )
        ...
        if task == "detect":
            list_of_channels = feature_sizes[0:number_heads]
            head = Detect(nc, ch=list_of_channels)
